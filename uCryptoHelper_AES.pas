...
var
  ...
  counterBE: DWORD; // big-endian counter
begin
  ...
  try
    ...
    begin
      ...
      if Length(Salt)>0 then
        Move(Salt[0], SaltCounter[0], Length(Salt));
      counterBE := Swap32(DWORD(i)); // převod i -> big-endian
      Move(counterBE, SaltCounter[Length(Salt)], SizeOf(counterBE));

      // U1 = HMAC(PW, Salt||i)
      U := HMAC_SHA256(PW, SaltCounter);
      TBlock := Copy(U, 0, Length(U));

      // U2..Uiter, XOR všech U do TBlock (32 bajtů)
    ...
    SetLength(Result, DKLen); // ořízni na požadovanou délku
  finally
    WipeBytes(PW);
  end;
end;
...
function BytesOfDWORDLE(Value: DWORD): TBytes; inline;
begin
  SetLength(Result, 4);
...
begin
  ...
  Mix := Concat2( Concat3(Salt, IV, BytesOfDWORDLE(IterLE)), Cipher );
